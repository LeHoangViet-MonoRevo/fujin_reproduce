    ...
    def build_process_graph_root(self) -> List:
        ...
        def build_process_graph(process_list, process_graph, layer):
            ...
            new_process_list = [
                ...
                not in self.build_existed_process_list(process_graph)
            ]
        ...
        for process in self.process_list:
            if (
                process["fixed"]
                and process["product_shape"]["shape"]
        ...
        if self.same_shape:
            ...
            process_match = [
                tuple(k["process_id"] for k in v) for v in process_match.values()
            ]
            process_match = sorted(
                process_match, key=lambda x: (-len(x), process_match.index(x))
            )
        else:
